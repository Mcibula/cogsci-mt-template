...
If you are reading this chapter already rendered in the PDF, make sure also to follow its source code located in
...
\verb|\tocite|. You can use \todo\dots, well, as a TO DO marker, and you can also use its parameterized version
\verb|\todo[something I have to do]| to leave comments for your future self. Similarly, you can \tocite to mark
places where you would like to add references later.
...
fragment the document too much. Paragraphs are also never numbered. If you would like to use headings without
...
\end{displayquote}
Alternatively, you can use a bold font via \verb|\textbf{text}|:
\begin{displayquote}
    \textbf{this is also important, but about as subtle as a sledgehammer}
\end{displayquote}
We generally avoid using underlining in academic writing, as it distorts the baseline of the font and can look quite
ugly.

Some characters in \LaTeX{} have special, reserved meanings: \verb|&|, \verb|%|, \verb|$|, \verb|#|, \verb|_|,
\verb|{|, \verb|}|, \verb|~|, \verb|^|, and \verb|\|. If you try to use these plainly in your text, \LaTeX{} will
usually throw a profane fit. For that reason, you need to \emph{escape} them by prepending them with a backslash,
like this: \&, \%, \$, \#, \_, \{, and \}. The only exceptions are \verb|~|, \verb|^|, and \verb|\|, which cannot be
escaped this way. Instead, use \verb|\textasciitilde|, \verb|\textasciicircum|, and \verb|\textbackslash|,
respectively. Note that \verb|\\| has the special function of forcing a line break, like this: \\ It is not
appropriate to use it in normal textual paragraphs, however.
...
\begin{itemize}
    ...
    \item \textbf{En-dash} (\verb|--|): Used for number ranges or as a parenthetical pause/digression,
    e.g., \verb|pages 10--20|.
    \item \textbf{Em-dash} (\verb|---|): Used as an unspaced parenthetical dash---like this---without spaces on
    either side.
...
As you have probably noticed by now, you can make a new paragraph by leaving a separating blank line in your editor.
Furthermore, \LaTeX{} ignores any indentation or multiple spaces next to each other; it uses a space only as a
signal that there is a new word following, but it prefers to manage the whitespace itself. Even if you start a new line
but do not leave a blank line in your source code, \LaTeX{} will continue typesetting in the same paragraph:
\begin{displayquote}
    This is Paragraph 1.
...
abbreviation right next to it. Optionally, you can also redeclare that abbreviation again on the first occurrence in
...
footnotes right next to each other; \LaTeX{} will automatically separate them with commas.} Note that it is not a
good idea to use footnotes in captions, as they are very sensitive to that kind of treatment and will very likely break.
...
\begin{equation}
    ...
    e^{i \pi} + 1 = 0.
\end{equation}
You can then refer to it in your text using \verb|\cref{eq:euler}|, which renders as \cref{eq:euler}. For a full
...
In case you use mathematical notation extensively, the template allows you to create a Summary of Notation.
...
While Bib\LaTeX{} is more advanced and allows us to use the full APA 7 style for references, it can also be
incredibly slow to compile, especially if you have a large bibliography and work in Overleaf. Hence, we will use
older but much faster Bib\TeX{}, which can only handle APA 6, but has been adjusted for this template to get as
close to APA 7 as possible.
...
\end{verbatim}
In the entry, \verb|@Article| determines the publication type; in this case, it is a journal article. You can
also use many others, e.g., \verb|@InProceedings|, \verb|@Book|, \verb|@InBook|, \verb|@PhdThesis|, \verb|@Misc|,
etc. Afterward, in this case, \verb|Harnad1990| is the identifier of the entry, so you can refer to it in the
document. The rest of the fields provide details about the publication. You can include extra fields, as Bib\TeX{}
will extract only the relevant ones and format them according to the APA guidelines. Unused references in
\verb|references.bib| will not be rendered in the final bibliography, and the rendered bibliography will be sorted
automatically.
...
create your own manually, use one of many online tools or your favorite AI friend to format the Bib\TeX{}
entry for you, or export it from any civilized reference manager (e.g., \emph{Zotero}, \emph{Mendeley}).
In case you are looking for a more complex solution, you can use, for example, the
\emph{\href{https://jabref.org/}{JabRef}} editor, which works directly with your \verb|references.bib|.

Additionally, there are a few stylistic recommendations for your bibliography entries you might consider:
\begin{enumerate}
    \item This template leaves the capitalization of the publication names mostly up to you. It is, though, advised to
    use sentence case for the names of articles, chapters, or any publications that are parts of larger works. For the
    names of journals, books, conferences and conference proceedings, theses, software, websites, or any other
    monographs, it is recommended to use title case. These names will also be rendered in italics, according to APA.
...
\end{verbatim}
    ...
    \item Do not specify the full date of an entry; leave just the year.
    \item Use en-dashes for page ranges in the \verb|pages| field.
    \item In the \verb|doi| field, use a pure DOI identifier, i.e., remove any prefixes such as \verb|doi:|,
    \verb|doi.org/|, or \verb|https://doi.org/|. With the APA 7 style, the DOI is the preferred identifier;
    no other will be displayed.
    \item If you are using a DOI, remove any other field including web addresses, such as \verb|url|.
    \item For online resources without a DOI, use the \verb|url| field and \verb|urldate| for retrieval dates
    formatted as \verb|1518-07-14|.
    \item You can include the journal or proceedings editors, but it is not necessary.
    \item You can remove all unnecessary fields, e.g., \verb|note|, \verb|eprint|, \verb|language|,
    \verb|version|, \verb|copyright|, \verb|keywords|, etc. These will never be rendered.
\end{enumerate}
...
Similar to inline quoting, you can include references with block quotes via the \verb|displaycquote| environment:
...
\end{displaycquote}
Note that we left out less important parts of the text using \verb|\textelp{}| to make the quote more focused
and shorter. Similarly, you can use \verb|\textins{...}| to provide clarifying comments in quotes or to indicate a
change in capitalization when you integrate a quote in your sentence:
\begin{displayquote}
...
All these elements are automatically numbered, indexed in the corresponding list at the beginning of your thesis,
and you can refer to them via standard \verb|\cref| commands. Whenever you use a figure, table, or algorithm,
also make sure to refer to it in your text, so they do not stand alone with only the minimal context provided by
their captions.
...
the \verb|\caption{}| command to ensure that cross-referencing links correctly. The caption should be descriptive,
explaining all visual elements, and can span multiple sentences. If you mention a specific visual element,
such as a dashed line, you can italicize it; in \cref{fig:instruments} we emphasized \emph{boldface}. The caption
will also be used automatically in the List of Figures as the name of the figure; however, captions are usually
awfully long for this purpose, so you can define a short name which will be used instead via
\verb|\caption[short name]{long caption}|.

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/instr-taste}
...
to the top of the page), \verb|b| (\emph{bottom} -- bottom of the page), and any combination of them, where the order
determines the priority. If you get desperate, you can additionally use \verb|!|, which will try to force \LaTeX{} to
comply; however, sometimes even that can be unsuccessful; in that case, try to move the figure to different places and
sections.

Regarding the actual images within your figures, make sure to use vector graphics whenever you are displaying plots,
charts, or diagrams. This ensures that your figures scale up properly under zoom or for print, and that any text
within them remains selectable and legible.\footnote{Make sure also to use an appropriate font size in your figures,
so the text remains visible even at 100\% zoom. While you can zoom in within a PDF viewer, readers of a printed
version will not have that luxury.} The easiest format to use for this purpose is PDF, which is supported by any proper
graphic software or plotting library. For photos, JPEG or PNG is fine.

\subsection{Tables}
\label{sec:tables}

This template uses the \emph{booktabs} table style, which avoids vertical lines and uses horizontal lines sparingly
to make tables cleaner. Similar to the \verb|figure| environment, you can define tables via the \verb|table|
...
and \verb|r| for alignment to the right). In the table, you separate columns with \verb|&| and lines with \verb|\\|;
any spacing around them is ignored. You usually do not need to input a table manually, as there are many web tools
able to generate it for you from any kind of table format (e.g.,
...
provided, in our case, by the \texttt{algorithm2e} package,\footnote{For more details on the usage of this package,
see \emph{\href{https://www.overleaf.com/learn/latex/Algorithms\#The\_algorithm2e\_package}{this tutorial}}.}
which has already been configured for this template. As with any pseudocode, you are free to use any combination
of natural language, math notation, and programming elements to communicate the idea as efficiently and clearly as
possible.

Same as with the previous floating environments, the \verb|algorithm| environment works the same in terms of
